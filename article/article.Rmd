...
The most important thing to note here is that `action_model` and `score_model` share all weights. This is important because we will not directly train the `score model`. Instead we will train the `action_model` using gradient descent and use the `score_model` just to get predictions of rewards for each action.
