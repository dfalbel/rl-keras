...
Gym does not document which action is performed by each integer value, but we can figure out by running actions in the emulator and looking at the rendered output. We can also get the number of possible actions by accessing the `env$action_space$n` member.

In our particular case, ie. the Atari Breakout game, there are 4 possible actions and the following table maps actions to corresponding integer values.
